...
import static com.flashcards.app.config.Constants.ROLE_USER;

public class FlashCardAuthenticator implements Authenticator<String, User> {

    private final SecretKey key;
    private final AuthDao authDao;

    public FlashCardAuthenticator(AuthDao authDao) {
        ...
        if (userFromDb.isPresent() && BCrypt.checkpw(password, userFromDb.get().getPasswordHash())) {
            ...
            return Optional.of(createToken(
                    new User(userFromDb.get().getUserId(),
                            userFromDb.get().getActiveCollectionId(),
